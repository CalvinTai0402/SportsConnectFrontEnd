...
export default function Portfolio() {
  ...
  useEffect(() => {
    if (!firstRuns.publicProfile.current)
      firstRuns.publicProfile.current = true;
    else if (!secondRuns.publicProfile.current)
      secondRuns.publicProfile.current = true;
  }, [publicProfile]);
  ...
  useEffect(() => {
    let fetchCurrentUser = async () => {
      let res;
      if (!userId) {
        res = await getCurrentUser(abortControllerRef.current);
      } else {
        res = await getUser(abortControllerRef.current, userId);
      }
      if (res?.status === 200) {
